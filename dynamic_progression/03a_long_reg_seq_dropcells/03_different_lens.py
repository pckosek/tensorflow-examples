...
def get_variables(constants):
  ...
  out = {
    ...
    'w_1' : tf.Variable(tf.random_normal(
      [constants['INPUT_TIME_STEPS']*constants['HIDDEN_DIM'],constants['OUTPUT_TIME_STEPS']]), 
      name='w_1'),
    'b_1' : tf.Variable(tf.random_normal(
      [constants['OUTPUT_DIM'],constants['OUTPUT_TIME_STEPS']]), 
      name='b_1')
...
def model(data, variables, constants, op='train'):
  ...
  rnn_layers = [
    rnn.GRUCell(
      ...
      activation=tf.nn.softsign) for _ in range(3)]
  
  dropcells = [rnn.DropoutWrapper(cell,input_keep_prob=dropout_pkeep) for cell in rnn_layers]

  multi_cell = tf.nn.rnn_cell.MultiRNNCell(dropcells)

  rnn_output, states = tf.nn.dynamic_rnn(multi_cell,
    input_reshape_op,
    time_major=False,
    dtype=tf.float32)

  stacked_rnn_output = tf.reshape(rnn_output, 
    [1, -1], name='stacked_rnn')
  ...
  reshape_out = tf.reshape(
    add_op,
    [-1, constants['OUTPUT_TIME_STEPS']] , name='reshout_2')
  ...
  return logits, labels, stacked_rnn_output
  # NLAYERS = 2
...
def train(train_steps):
  ...
  # logits
  logits, labels, stack = model(data, variables, constants, op='train')
  ...
  with tf.Session() as sess:
    ...
    for i in range(train_steps) :
      ...
      if ((i+1)%499 == 0):
          ...
          st = sess.run(stack)
          eng.workspace['stack'] = matlab.double( np.asarray(st).tolist() ) 
...
def test(test_steps):
  ...
  # logits
  logits, labels, stack = model(data, variables, constants, op='test')
